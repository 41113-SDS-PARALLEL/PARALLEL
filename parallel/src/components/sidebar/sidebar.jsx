...
class Sidebar extends Component {
    ...
    handleAddStream = () => {
        ...
        setTimeout(() => {
            if (this.sidebarRef.current) {
                this.sidebarRef.current.scrollTop = this.sidebarRef.current.scrollHeight;
            }
        }, 0);
    };
    ...
    componentDidUpdate(prevProps, prevState) {
        if (!prevState.optionsStream && this.state.optionsStream) {
            if (this.sidebarRef.current) {
                this.sidebarRef.current.addEventListener('wheel', this.preventScroll, { passive: false });
                this.sidebarRef.current.addEventListener('touchmove', this.preventScroll, { passive: false });
                this.sidebarRef.current.addEventListener('keydown', this.preventKeyScroll, { passive: false });
            }
        } else if (prevState.optionsStream && !this.state.optionsStream) {
            if (this.sidebarRef.current) {
                this.sidebarRef.current.removeEventListener('wheel', this.preventScroll);
                this.sidebarRef.current.removeEventListener('touchmove', this.preventScroll);
                this.sidebarRef.current.removeEventListener('keydown', this.preventKeyScroll);
            }
        }
    }
    ...
    render() { 
        const { ref, navigateToDate, streams, onSelectStream, onEditStream, onDeleteStream, colors, events } = this.props;
        return (
            <div
                id="Sidebar"
                ref={this.sidebarRef}
            >
                ...
                <div id='streams'>
                    ...
                    <StreamList 
                        ...
                        onOptionsClick={streamID => this.setState({ optionsStream: streamID, optionsPosition: this.calculateStreamOptionsPosition(this.sidebarRef.current.querySelector(`button[name="stream-options-button-${streamID}"]`)) })}
                    />
                    {this.state.optionsStream && (
                        <StreamOptions 
                            stream={streams.find(stream => stream.id === this.state.optionsStream)}
