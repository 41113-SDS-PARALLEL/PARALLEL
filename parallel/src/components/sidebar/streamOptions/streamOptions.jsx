...
class StreamOptions extends Component {
    ...
    render() {
        const { stream, onClose, onDeleteStream, onEditStream, colors, position, streams, events } = this.props;
        return (
            <div className='stream-options' ref={this.streamOptionsRef} style={{ top: position.top, left: position.left }}> 
                <div className='stream-options-name'>
                    {this.state.editingName ? (
                        <input
                            ...
                            value={stream.name}
                            onChange={(e) => {onEditStream(stream.id, e.target.value, stream.color)}}
                            onBlur={() => {this.setState({ editingName: false })}}
                            onKeyDown={(e) => {if (e.key === 'Enter') {this.setState({ editingName: false })}}}
                            autoFocus
                        />
                    ) : (
                        <>
                ...
                <div className='color-grid'>
                    {colors.map((color, index) => (
                        <button
                            ...
                            onClick={() => {onEditStream(stream.id, stream.name, color)}}
                            style={{ backgroundColor: color, border: stream.color === color ? '2px solid black' : 'none' }}
                        />
                    ))}
                </div>
                <div className='stream-options-actions'>
                    <button
                        ...
                        onClick={() => {console.log('Edit Times clicked');}}
                    >
                        Edit Times
                    </button>
                    <button
                        ...
                        onClick={() => {
                            const hasEvents = events && events.some(e => e.extendedProps.stream === stream.id);
                            if (hasEvents) {
